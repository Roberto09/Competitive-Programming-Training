...
bool visited[100050];
void disjktra(vector<node> &graph, ll minDist[], ll xd[], ll src){
	...
	while(!pq.empty()){
		...
		if(visited[curr]) continue; // we don't want to process it several times, we should ignore it instead
		visited[curr] = true;
		FOR(i, 0, graph[curr].neig.size()){
			...
			else if(currDist + neigDist == minDist[neig]){
				xd[neig] = (xd[neig] + xd[curr]) % MOD;
			}
		}
	}
}
...
int main(){
	...
	xd[0] = 1;
	disjktra(graph, minDist, xd, 0);
	cout << minDist[nodes-1] << " " << xd[nodes-1] << endl;
	// for (int i = 0; i < nodes; ++i)
	// {
	// 	cout << minDist[i] << " " << xd[i] << endl;
	// }
	return 0;
}
