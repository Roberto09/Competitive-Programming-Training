...
struct vrt{
	...
	char chr;
	vrt(){
		leaf = false; prnt = -1; sufixLink = -1; endWordLink = -1;
	}
};
...
ll processString(string  &text, char ind, vii &coords){
	ll currState = root;
	ll res = 0;
	FOR(i, 0, text.length()){ // iteramos sobre los caracteres de text
		...
		while(1){
			...
			ress[trie[checkState].wordId] = ind;
			ressp[trie[checkState].wordId] = coords[i+1-words[trie[checkState].wordId].length()];
...
void checkSp(ll r, ll c, ii mv, char C){
	ll i = r, j = c;
	vii coords;
	string pr = "";
	while(i < n && j < m && i >= 0 && j >= 0){
		coords.pb(mp(i, j));
		pr += mtrx[i][j];
		i += mv.first; j += mv.second;
	}
	//processString
	processString(pr, C, coords);
}

void solve(){
	FOR(i, 0, n){
		checkSp(i, 0, mp(0, 1), 'C'); checkSp(i, 0, mp(1, 1), 'D'); checkSp(i, 0, mp(-1, 1), 'B');
	}
	FOR(i, 0, m){
		checkSp(0, i, mp(1, 0), 'E'); checkSp(0, i, mp(1, -1), 'F'); checkSp(0, i, mp(1, 1), 'D');
	}
	FOR(i, 0, n){
		checkSp(i, m-1, mp(0, -1), 'G'); checkSp(i, m-1, mp(-1, -1), 'H'); checkSp(i, m-1, mp(1, -1), 'F'); 
	}
	FOR(i, 0, m){
		checkSp(n-1, i, mp(-1, 0), 'A'); checkSp(n-1, i, mp(-1, 1), 'B'); checkSp(n-1, i, mp(-1, -1), 'H'); 
	}
}

void doAlg(){
	words.clear(); trie.clear(); ress.clear(); ressp.clear(); sz = 0;
	initAho();
	cin >> n >> m >> k;
	FOR(i, 0, n) cin >> mtrx[i];
	FOR(i, 0, k){
		string s;
		cin >> s;
		words.pb(s);
		ress.pb(' '); ressp.pb(mp(0, 0));
		addString(s, i);
	}
	prepareAho();
	solve();
	FOR(i, 0, k){
		cout << ressp[i].first << " " << ressp[i].second << " " << ress[i] << etr;
	}
	cout << etr;
}
